...
volatile uint8_t OV5640_FrameState =
    0; // DCMI状态标志，当数据帧传输完成时，会被 HAL_DCMI_FrameEventCallback() 中断回调函数置 1
...
{
    uint16_t x_off,
             y_off; // 水平和垂直偏移，垂直代表的是行数，水平代表的是像素时钟数（PCLK周期数）
    ...
    HAL_DCMI_ConfigCrop(&hdcmi, x_off, y_off, dcmi_capcnt,
                        dcmi_vline); // 设置裁剪窗口
    HAL_DCMI_EnableCrop(
        &hdcmi);                                                        // 使能裁剪
...
void ov5640_reset(void)
{
    ...
    sccb_write_reg_16bit(0x3103,
                         0x11); // 根据手册的建议，复位之前，直接将时钟输入引脚的时钟作为主时钟
...
void ov5640_para_set(void)
{
    ...
    {
        sccb_write_reg_16bit(OV5640_INIT_Config[i][0],
                             OV5640_INIT_Config[i][1]); // 写入配置
        ov5640_delay(1);
...
void ov5640_set_pixformat(uint8_t pixformat)
{
    ...
    {
        sccb_write_reg_16bit(OV5640_FORMAT_CONTROL,
                             0x30);     //   设置数据接口输出的格式
        ...
        sccb_write_reg_16bit(OV5640_JPEG_VFIFO_HSIZE_H,
                             OV5640_Width >> 8);      // JPEG输出水平尺寸,高字节
        sccb_write_reg_16bit(OV5640_JPEG_VFIFO_HSIZE_L,
                             (uint8_t)OV5640_Width);  // JPEG输出水平尺寸,低字节
        sccb_write_reg_16bit(OV5640_JPEG_VFIFO_VSIZE_H,
                             OV5640_Height >> 8);     // JPEG输出垂直尺寸,低字节
        sccb_write_reg_16bit(OV5640_JPEG_VFIFO_VSIZE_L,
                             (uint8_t)OV5640_Height); // JPEG输出垂直尺寸,低字节
    }
    ...
    {
        sccb_write_reg_16bit(OV5640_FORMAT_CONTROL,
                             0x60);     // 此处设置为RGB565格式，序列为 G[2:0]B[4:0], R[4:0]G[5:3]
        sccb_write_reg_16bit(OV5640_FORMAT_CONTROL_MUX, 0x01); // 设置ISP的格式
    }

    reg = sccb_read_reg_16bit(0x3821); // 读取寄存器值，Bit[5]用于是否使能JPEG模式
    sccb_write_reg_16bit(0x3821,
    (reg & 0xDF) | ((pixformat == Pixformat_JPEG) ? 0x20 : 0x00));

    reg = sccb_read_reg_16bit(
              0x3002); // 读取寄存器值，Bit[7]、Bit[4]和Bit[2]使能 VFIFO、JFIFO、JPG
    sccb_write_reg_16bit(0x3002,
    (reg & 0xE3) | ((pixformat == Pixformat_JPEG) ? 0x00 : 0x1C));

    reg = sccb_read_reg_16bit(
              0x3006); // 读取寄存器值，Bit[5]和Bit[3] 用于是否使能JPG时钟
    sccb_write_reg_16bit(0x3006,
...
int8_t ov5640_set_framesize(uint16_t width, uint16_t height)
{
    ...
    sccb_write_reg_16bit(OV5640_TIMING_DVPHO_H,
                         width >> 8); // DVPHO，设置输出水平尺寸
    sccb_write_reg_16bit(OV5640_TIMING_DVPHO_L, width & 0xff);
    sccb_write_reg_16bit(OV5640_TIMING_DVPVO_H,
                         height >> 8); // DVPVO，设置输出垂直尺寸
...
int8_t ov5640_af_download_firmware(void)
{
    ...
    uint16_t i = 0;                    // 计数变量
    uint16_t OV5640_MCU_Addr =
        0x8000; // OV5640 MCU 存储器的起始地址为 0x8000，大小为4KB

    sccb_write_reg_16bit(0x3000,
                         0x20); // Bit[5]，复位MCU，写入固件之前，需要执行此操作
    // 开始写入固件，批量写入，提高写入速度
    sccb_write_buf_16bit(OV5640_MCU_Addr, (uint8_t *)OV5640_AF_Firmware,
    ...
    for (i = 0; i < 100; i++)
    {
        ...
        if (af_reg ==
                0x70) // 释放马达，镜头回到初始（对焦为无穷远处）位置，意味着固件写入成功
        {
